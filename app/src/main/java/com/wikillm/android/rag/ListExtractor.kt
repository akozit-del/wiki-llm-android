...
class ListExtractor(
    ...
    private val mapSystem =
        "Ты извлекаешь факты из ОДНОЙ выдержки Википедии. " +
            "Верни ТОЛЬКО тех людей, кто в этой выдержке прямо назван должностью из вопроса " +
            "(именно этой должностью, не другой). Формат: по одной строке «Имя — годы». " +
            "Если годы не указаны — «Имя — ?». Если в выдержке нет подходящего человека — " +
            "ответь ровно «НЕТ». Без пояснений, без преамбулы, без Markdown."
