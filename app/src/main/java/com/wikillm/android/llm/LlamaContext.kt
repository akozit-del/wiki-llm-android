...
/**
 * Kotlin-side callback that the JNI layer calls for each token.
 * Return `false` to ask the native code to stop early.
 */
...
class LlamaContext private constructor(private val handle: Long) : AutoCloseable {
    ...
    /** Streams generated text pieces. The flow completes when generation ends. */
    fun generate(prompt: String, maxTokens: Int = 256): Flow<String> = channelFlow {
        ...
        // Run blocking JNI work on IO dispatcher. When the consumer cancels
        // the flow, channel closes, callback returns false, native loop exits.
